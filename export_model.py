...
class TextHeadWithProj(torch.nn.Module):
    def __init__(self, text_model_name: str, output_dim: int):
        ...
        self.fc = torch.nn.Linear(self.backbone.transformer.layer[-1].ffn.lin2.out_features, output_dim)

    def forward(self, input_ids, attention_mask):
        embedding = self.backbone(input_ids, attention_mask).last_hidden_state
...
def main():
    ...
    tokenizer.save_pretrained("assets/distilbert-tokenizer")
    model.eval()
    ...
    ort_session = ort.InferenceSession("distilbert_embedder.onnx")
